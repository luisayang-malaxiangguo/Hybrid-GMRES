function [x, err, res, niters, phi, dPhi] = BAgmres_nonhybrid_bounds( ...
    A, B, b, x_true, tol, maxit, DeltaM)
% BAGMRES_NONHYBRID_BOUNDS_PATCHED Non-hybrid BA-GMRES + fully corrected perturbation bounds.
% PATCH NOTES:
...
%   2. `dMu` is now computed in the same harmonic-Ritz basis `W` as `dTheta`.
    ...
    % For M=BA, the perturbation on M is dM = DeltaB*A
    dKk = Qk' * (DeltaM * A * Qk);
